...
Ropebwt3 indexed 320 assembled human genomes in 81 hours and indexed 7.3 terabases of commonly studied bacterial assemblies in 26 days.
This was achieved using 100 gigabytes of memory at the peak without working disk space.
Ropebwt3 can find maximal exact matches and inexact alignments under affine-gap penalties,
and can retrieve all distinct local haplotypes matching a query sequence.
...
On average, $k\approx s$.

{\sc Locate1} in Algorithm~\ref{algo:locate} provides a faster way to find one position in SA interval $[l,h)$.
		...
		\Procedure{IndexSSA}{$B,s$}
			...
			\For{$t\gets 0$ {\bf to} $m-1$}\Comment{Traverse all sequences}
				...
				\Repeat\Comment{Iterate from the end of the sequence}
					...
					\If{$B[k]=\$$}
						\State $R(k)\gets t$\Comment{The rank of $t$-th sequence}
					\ElsIf{$k \bmod s=0$}
...
For example, given $T={\tt CAAGCA}$ and $P={\tt AGC}$,
...
\toprule
Dataset        & Algorithm     & Elapsed$^1$ & CPU$^1$  &     RAM$^2$ \\
\midrule
human100       & grlBWT        &  8.3 h       &  29.6 h &  84.8 GB \\
		       & pfp-thres$^3$ &$<$51.7 h     &$<$51.5 h & 788.1 GB \\
               & ropebwt3      & 20.5 h       & 469.4 h &  83.0 GB \\
			   & metagraph$^4$ & 16.9 h       & 314.8 h & 251.0 GB \\
			   & fulgor$^5$    & 1.2 h    &  32.7 h & 165.2 GB \\
human320       & grlBWT        & 24.4 h   &  97.4 h & 270.4 GB \\
               & ropebwt3      & 81.2 h       & 1343.4 h & 99.2 GB \\
...
\item[$^{2}$] 1 GB is equal to $1024^3$ bytes
...
To test the scalability of ropebwt3, we indexed a large dataset consisting of 320 human genomes.
Ropebwt3 took 3.4 days and generated a 13.1 GB file in the fermi format.
It contains 1.9 trillion bases and 4.9 billion runs.
We also constructed the BWT of AllTheBacteria~\citep{Hunt2024.03.08.584059} excluding the ``dustbin'' and ``unknown'' categories that include relatively unique genomes.
This subset, which we call CommonBacteria, consists of 278.4 million sequences.
...
Ropebwt3 is likely to take $\sim$3 weeks to index 1,000 human genomes assembled from long reads.
...
some of their data structures are probably better than the ones we use.
