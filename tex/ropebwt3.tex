...
The basic idea is well known~\citep{DBLP:conf/latin/FerraginaGM10},
but different from existing implementations,
ropebwt3 uses a B+-tree to encode BWT (Fig.~\ref{fig:2}a)
for $O(\log r)$ insertion on line 14.
The time complexity of Algorithm~\ref{algo:merge} is $O(|B_2|\log r)$
where $r$ is the number of runs in the merged BWT.
The space complexity is $O(r\log r+|B_2|\log n)$.
The loop on line 4 calculates ranks.
It does not modify $B_1$ or $B_2$ and can be parallelized if $m_2>1$.
The batch insertion on line 13 is sequential and is much faster than rank calculation in practice.
...
On real data, $G_T$ is often too large to store explicitly.
We instead explicitly store $G_P$ only and travese it in the topological order (Algorithm~\ref{algo:bwa-sw}).
At a node $u\in V(G_P)$, we use a hash table to keep $\{v\in V(G_T):H_{uv}>0\}$.
This algorithm is exact in that it guarantees to the best alignment.
